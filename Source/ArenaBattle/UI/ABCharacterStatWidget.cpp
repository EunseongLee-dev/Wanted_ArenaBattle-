...
void UABCharacterStatWidget::UpdateStat(const FABCharacterStat& BaseStat, const FABCharacterStat& ModifierStat)
{
		...
		++PropIt)
	{
		...
		float BaseDate = 0.0f;
		PropIt->GetValue_InContainer(reinterpret_cast<const void*>(&BaseStat), &BaseDate);

		// 리플렉션을 활용해 변수의 값 읽어오기
		float ModifierDate = 0.0f;
		PropIt->GetValue_InContainer(reinterpret_cast<const void*>(&ModifierStat), &ModifierDate);
		...
		if (BaseTextBlockPtr)
		{
			...
			FString FloatValue = FString::SanitizeFloat(BaseDate);
		...
		{
			// Float 값을 FString으로 변환
			FString FloatValue = FString::SanitizeFloat(ModifierDate);
